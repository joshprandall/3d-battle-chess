...
export function createPiece(piece,x,y,theme='classic'){
 ...
 const edge=new THREE.MeshStandardMaterial({color:dark?0xc9e3f1:p.trim,metalness:.45,roughness:.29,emissive:dark?0x34566c:p.glow,emissiveIntensity:dark?.22:.15});
 const glow=new THREE.MeshStandardMaterial({color:p.glow,emissive:p.glow,emissiveIntensity:1.25,roughness:.24});
 const ink=new THREE.MeshStandardMaterial({color:dark?0x132234:0x354352,roughness:.5});
 const other=new THREE.MeshStandardMaterial({color:dark?0x9bb4c4:0xb18a61,metalness:.35,roughness:.42});
