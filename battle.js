...
import {chooseComputerMoveV7} from './computer-v7.js';
import {createChessAudio} from './audio-v7.js';
import {createExperience} from './experience-v7.js';
...
let theme='classic',selected=null,legal=[],busy=false,aiTimer=null,generation=0,toastTimer=null,scene,camera,renderer,orbit,boardGroup,pieceGroup,fxGroup,experience;
...
function connectButtons(){$('#newGame').onclick=newGame;$('#undo').onclick=()=>{if(busy)return;generation++;clearTimeout(aiTimer);if(!game.undo()){notice('No move to undo.');return}if($('#mode').value==='ai'&&game.turn==='b')game.undo();selected=null;legal=[];drawPieces();renderStatus()};$('#flip').onclick=()=>{if(camera&&orbit){camera.position.x*=-1;camera.position.z*=-1;orbit.update()}else notice('3D camera unavailable.');};$('#sound').onclick=()=>{audio.setEnabled(!audio.enabled);experience?.updateSound()};$('#theme').onchange=e=>{if(busy){e.target.value=theme;return}theme=e.target.value;if(boardGroup)createBoard();if(pieceGroup)drawPieces();experience?.update()};$('#mode').onchange=newGame;$('#difficulty').onchange=e=>{notice(['','Recruit · varied legal moves','Warrior · looks one move ahead','Champion · checks opponent replies'][Number(e.target.value)]+' · applies on next computer turn')};$('#menuBtn').onclick=e=>{const c=$('#controls'),open=c.classList.toggle('open');e.currentTarget.setAttribute('aria-expanded',String(open))}}
function init(){experience=createExperience({game,sceneEl,pick:chooseSquare,cancel:()=>{selected=null;legal=[];if(boardGroup)highlight();experience.update();},undo:()=>$('#undo').click(),flip:()=>$('#flip').click(),audio,getSelection:()=>selected,getLegal:()=>legal,onCursor:()=>{if(boardGroup)highlight();},castleAttempt});connectButtons();connectKeyboardMoves();try{scene=new THREE.Scene();scene.fog=new THREE.Fog(themes[theme].back,15,35);camera=new THREE.PerspectiveCamera(43,1,.1,100);camera.position.set(8.5,10,9.5);renderer=new THREE.WebGLRenderer({antialias:true,alpha:true});renderer.setPixelRatio(Math.min(devicePixelRatio||1,2));renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=.84;sceneEl.appendChild(renderer.domElement)}catch(error){stateEl.textContent='3D graphics unavailable';sceneEl.querySelector('canvas')?.remove();experience.setAvailable(false);stateEl.textContent='2D board ready';renderStatus();notice('3D unavailable; you can still play in 2D.');return}orbit=new OrbitControls(camera,renderer.domElement);orbit.enableDamping=true;orbit.target.set(0,.25,0);orbit.minDistance=7;orbit.maxDistance=22;orbit.maxPolarAngle=1.47;orbit.update();scene.add(new THREE.HemisphereLight(0xdcecf8,0x718397,1.15));const light=new THREE.DirectionalLight(0xffedd3,2.0);light.position.set(6,12,5);light.castShadow=true;light.shadow.mapSize.set(1024,1024);light.shadow.bias=-.00012;light.shadow.normalBias=.018;light.shadow.radius=2.4;scene.add(light);const rim=new THREE.DirectionalLight(0xb2ecff,1.15);rim.position.set(-5,8,-7);scene.add(rim);const fill=new THREE.DirectionalLight(0xffffff,.40);fill.position.set(-6,4,6);scene.add(fill);boardGroup=new THREE.Group();pieceGroup=new THREE.Group();fxGroup=new THREE.Group();scene.add(boardGroup,pieceGroup,fxGroup);createBoard();drawPieces();renderStatus();connectPointers();const resize=()=>{const w=Math.max(1,sceneEl.clientWidth),h=Math.max(1,sceneEl.clientHeight);renderer.setSize(w,h,false);camera.aspect=w/h;camera.fov=camera.aspect<.85?62:43;camera.updateProjectionMatrix()};new ResizeObserver(resize).observe(sceneEl);resize();renderer.setAnimationLoop(()=>{orbit.update();renderer.render(scene,camera)})}
init();
