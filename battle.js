...
function createBoard(){
 ...
 sceneEl.style.background=`radial-gradient(ellipse at 50% 27%, ${new THREE.Color(p.back).offsetHSL(0,0,.17).getStyle()} 0%, ${new THREE.Color(p.back).getStyle()} 55%, #101923 100%)`;
 ...
 const base=new THREE.Mesh(new THREE.BoxGeometry(9,.36,9),material(0x243748));base.position.y=-.25;base.receiveShadow=true;boardGroup.add(base);
...
function init(){try{scene=new THREE.Scene();scene.fog=new THREE.Fog(themes[theme].back,15,35);camera=new THREE.PerspectiveCamera(43,1,.1,100);camera.position.set(8.5,10,9.5);renderer=new THREE.WebGLRenderer({antialias:true,alpha:true});renderer.setPixelRatio(Math.min(devicePixelRatio||1,2));renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;sceneEl.appendChild(renderer.domElement)}catch(error){stateEl.textContent='3D graphics unavailable';sceneEl.textContent='This browser could not start WebGL. '+error.message;return}orbit=new OrbitControls(camera,renderer.domElement);orbit.enableDamping=true;orbit.target.set(0,.25,0);orbit.minDistance=7;orbit.maxDistance=22;orbit.maxPolarAngle=1.47;orbit.update();scene.add(new THREE.HemisphereLight(0xe9f6ff,0x9bafc6,3.1));const light=new THREE.DirectionalLight(0xffefd9,3.5);light.position.set(6,12,5);light.castShadow=true;light.shadow.mapSize.set(1024,1024);scene.add(light);const rim=new THREE.DirectionalLight(0xb2ecff,3.8);rim.position.set(-5,8,-7);scene.add(rim);const fill=new THREE.DirectionalLight(0xffffff,1.15);fill.position.set(-6,4,6);scene.add(fill);boardGroup=new THREE.Group();pieceGroup=new THREE.Group();fxGroup=new THREE.Group();scene.add(boardGroup,pieceGroup,fxGroup);createBoard();drawPieces();renderStatus();connectPointers();connectButtons();const resize=()=>{const w=Math.max(1,sceneEl.clientWidth),h=Math.max(1,sceneEl.clientHeight);renderer.setSize(w,h,false);camera.aspect=w/h;camera.fov=camera.aspect<.85?62:43;camera.updateProjectionMatrix()};new ResizeObserver(resize).observe(sceneEl);resize();renderer.setAnimationLoop(()=>{orbit.update();renderer.render(scene,camera)})}
init();
