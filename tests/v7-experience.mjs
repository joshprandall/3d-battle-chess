...
try{
 ...
 await page.waitForFunction(()=>document.querySelectorAll('#log li').length===1,null,{timeout:5000}).catch(async err=>{throw Error(`2D e2e4 failed: ${JSON.stringify(await page.evaluate(()=>({log:document.querySelector('#log').innerText,from:document.querySelector('[data-square="e2"]').className,to:document.querySelector('[data-square="e4"]').className,toast:document.querySelector('#toast').textContent,state:document.querySelector('#state').textContent,fullscreen:!!document.fullscreenElement})))}; ${err.message}`)});
