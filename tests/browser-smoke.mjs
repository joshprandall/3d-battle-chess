...
try{
 ...
 assert.ok(await page.locator('#setupScreen').isVisible(),'setup screen is the initial game experience');
 assert.equal(await page.locator('#scene canvas').count(),0,'3D board does not initialize before Start');
 assert.equal(await page.locator('.site-nav').count(),0,'redundant portfolio navigation is absent inside the game');
 ...
 assert.equal(await page.locator('#scene canvas').count(),1,'WebGL board canvas exists after Start');
 assert.equal(await page.locator('#gameShell').getAttribute('data-fullscreen-requested'),'yes','Start requests immersive fullscreen');
 ...
 assert.equal(await page.locator('.site-nav').count(),0,'portfolio navigation stays absent at mobile width');
